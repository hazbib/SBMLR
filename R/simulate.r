...
if (p["mod"]==1)   v[j]=m[mi$rIDs[j]]*model$reactions[[j]]$law(St[c(model$reactions[[j]]$reactants,model$reactions[[j]]$modifiers)],model$reactions[[j]]$parameters)
if (p["mod"]==2)   v[j]=modulator[[mi$rIDs[j]]](t)*model$reactions[[j]]$law(St[c(model$reactions[[j]]$reactants,model$reactions[[j]]$modifiers)],model$reactions[[j]]$parameters)
}
xp=mi$incid%*%v
