  ...
  {  # takes SBML in filename.xml and maps it to a SBML class model 
    ...
    { # first block here sets up the parent environment used by all handler functions
      ...
      .endElement <- function(name) {
          if(name=="listOfFunctionDefinitions")  funcDef <<-FALSE  
...
#          
         ...
         if("initialConcentration" %in% names(out)){
            out$initialConcentration <- as.numeric(out$initialConcentration)      
          }else{
...
#    nFunctions=length(functions)
    if (nFunctions>0){
      for (i in 1:nFunctions)
      {
        # so far this is an expression
        model$functions[[i]] <- mathml2R(functions[[i]][["math"]][[1]])
      }
    }
    ...
    cat("Number of funtions: " , nFunctions, "\n")
